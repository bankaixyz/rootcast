...
NETWORK="sepolia"
ACCOUNT="${STARKNET_SEPOLIA_ACCOUNT_NAME:-world-id-root-replicator-starknet-sepolia}"
VK="${PROGRAM_VKEY:-${WORLD_ID_ROOT_REPLICATOR_PROGRAM_VKEY:-}}"
RPC_URL="${STARKNET_SEPOLIA_RPC_URL:-${STARKNET_SEPOLIA_RPC:-}}"
SNCAST_BIN="${SNCAST_BIN:-sncast}"

usage() {
  cat <<'EOF'
Usage: deploy.sh [--chain starknet-sepolia] [--account NAME] [--program-vkey BYTES32]

Deploy the Starknet Sepolia root registry after the contract is already built.
EOF
}
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --account)
      ...
      ;;
    --program-vkey)
      VK=${2:?missing value for --program-vkey}
      shift 2
      ;;
    --help|-h)
      usage
...
normalize_rpc_url() {
  ...
  if echo "$url" | grep -q 'quiknode\.pro' &&
    ! echo "$url" | grep -q '/rpc/v0_'; then
    url="${url%/}/rpc/v0_10"
  fi
...
run_sncast() {
  ...
  if [[ "$status" -ne 0 ]] && [[ -n "$RPC_URL" ]] &&
    echo "$output" | grep -Eq 'Invalid block id|uses incompatible version'; then
...
if [[ -z "$ACCOUNT" ]]; then
  echo "STARKNET_SEPOLIA_ACCOUNT_NAME must be set or passed with --account" >&2
  exit 1
fi

if [[ -z "$VK" ]]; then
  echo "PROGRAM_VKEY or WORLD_ID_ROOT_REPLICATOR_PROGRAM_VKEY must be set" >&2
  exit 1
fi
...
if [[ "$DECLARE_STATUS" -ne 0 ]] &&
  ! echo "$DECLARE_OUTPUT" | grep -q "is already declared"; then
  echo "Declare failed"
  echo "$DECLARE_OUTPUT"
  exit 1
fi
...
if echo "$DECLARE_OUTPUT" | grep -q "is already declared"; then
  ...
  CLASS_HASH=$(echo "$DECLARE_OUTPUT" |
    sed -nE 's/^(class_hash:|Class Hash:)[[:space:]]*(0x[0-9a-fA-F]+)$/\2/p' |
    head -1)
fi
...
run_sncast DEPLOY_OUTPUT DEPLOY_STATUS deploy \
  --class-hash "$CLASS_HASH" \
  --constructor-calldata 0x$VK_LOW 0x$VK_HIGH
CONTRACT_ADDRESS=$(echo "$DEPLOY_OUTPUT" |
  sed -nE 's/^(contract_address:|Contract Address:)[[:space:]]*(0x[0-9a-fA-F]+)$/\2/p' |
  head -1)
